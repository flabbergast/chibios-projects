...
static THD_FUNCTION(BreatheThread, arg) {
  ...
  while(true) {
    // orange led on F072-DISCO is on PC8, TIM3_CH3, AF0
    pwmEnableChannel(&PWM_DRIVER, 3-1, PWM_PERCENTAGE_TO_WIDTH(&PWM_DRIVER,breathing_table[table_pos]));
    table_pos = (table_pos+1) % TABLE_SIZE;
    chThdSleepMilliseconds(BREATHE_STEP);
  }
}

// TODO: PWM_PERCENTAGE_TO_WIDTH
...
int main(void) {
  ...
  /*
   * Start the PWM driver, route output to onboard LED.
   */
  pwmStart(&PWM_DRIVER, &pwmcfg);
  // orange led on F072-DISCO is on PC8, TIM3_CH3, AF0
  palSetPadMode(GPIOC, GPIOC_LED_ORANGE, PAL_MODE_ALTERNATE(0));
